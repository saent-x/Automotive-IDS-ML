...
class Testing:
    ...
    def generate_all_test_metrics(self, true_values_and_predicted_values) -> List[Tuple[str, Any, Any, Any]]:
        resulting_metrics = []
        (true_values, predicted_values) = true_values_and_predicted_values

        for i in range(len(true_values)):
            y_true = true_values[i]
            y_pred = predicted_values[i]

            class_report, df, cm = generate_metrics_report(y_true, y_pred)
    ...
    def save_testing_results_2(self, resulting_metrics: List[Tuple[str, Any, Any, Any]], title: str, attack: str):
        filename = f"{title.replace(' ', '_').replace('-', '').lower()}_results.txt"
